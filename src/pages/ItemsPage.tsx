...
import { generateId, handleImgError, formatCurrency } from '../utils/helpers'
...
export default function ItemsPage({ items, setItems, categories, showToast }: Props) {
  const [form, setForm] = useState({ itemName: '', price: '', quantity: '', categoryName: '' })
  ...
  const handleAdd = () => {
    ...
    // addItem({ ...form, price: +form.price, quantity: +form.quantity })
    //   .then((res) => setItems((p) => [...p, res.data]))
    ...
    setTimeout(() => {
      ...
      setForm({ itemName: '', price: '', quantity: '', categoryName: '' })
      setLoading(false)
      showToast('Item added successfully!')
    }, 600)
  }

  const categoryOptions = categories.map((c) => ({ value: c.categoryName, label: c.categoryName }))
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="card animate-fade-up">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 items-end">
          <FormInput label="Item Name"  placeholder="e.g. Coke"  value={form.itemName}     onChange={(e) => setForm({ ...form, itemName: e.target.value })}     required />
          <FormInput label="Price (₹)"  type="number" placeholder="40"  value={form.price}        onChange={(e) => setForm({ ...form, price: e.target.value })}         required />
          <FormInput label="Quantity"   type="number" placeholder="50"  value={form.quantity}     onChange={(e) => setForm({ ...form, quantity: e.target.value })} />
          <FormSelect label="Category"  options={categoryOptions}       value={form.categoryName} onChange={(e) => setForm({ ...form, categoryName: e.target.value })} />
          <button className="btn-primary w-full" onClick={handleAdd} disabled={loading}>
            <Plus size={15} />{loading ? '…' : 'Add Item'}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input className="form-input pl-10" placeholder="Search items or categories…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <div className="flex gap-1.5 bg-white border border-ink-200 rounded-xl p-1">
          <button className={view === 'grid' ? 'btn-primary !px-3.5 !py-1.5 !text-xs !rounded-lg' : '!px-3.5 !py-1.5 text-xs text-ink-400 rounded-lg hover:text-ink-700 border-0 bg-transparent cursor-pointer font-medium'} onClick={() => setView('grid')}>⊞ Grid</button>
          <button className={view === 'list' ? 'btn-primary !px-3.5 !py-1.5 !text-xs !rounded-lg' : '!px-3.5 !py-1.5 text-xs text-ink-400 rounded-lg hover:text-ink-700 border-0 bg-transparent cursor-pointer font-medium'} onClick={() => setView('list')}>≡ List</button>
        </div>
            ...
            {filtered.map((item) => (
              <div key={item.id} className="item-card animate-fade-up">
                <div className="relative h-[148px] overflow-hidden bg-ink-50">
                  <img src={item.image} alt={item.itemName} className="w-full h-full object-cover" onError={handleImgError} />
                    ...
                    <tr key={item.id} className="table-row">
                      <td className="td-cell">
                        <div className="flex items-center gap-2.5">
                          <img src={item.image} alt={item.itemName} className="w-10 h-10 rounded-lg object-cover flex-shrink-0 border border-ink-100" onError={handleImgError} />
